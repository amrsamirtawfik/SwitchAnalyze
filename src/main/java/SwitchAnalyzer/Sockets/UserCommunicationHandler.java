...
public class UserCommunicationHandler {
    ...
    public static void listenToCommands(WebSocketServer server , int maxMessageLength, Queue<Command> cmdQueue)  {
        while (true) {
    ...
    private static void parseCommand(byte[] command, Queue<Command> cmdQueue) throws JsonProcessingException {
        String jsonStr = new String(command);
        ObjectMapper mapper = new ObjectMapper();
        Command c = mapper.readValue(jsonStr, Command.class);
        cmdQueue.add(c);
    }
